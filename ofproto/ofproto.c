...
{
    if (rule->cr.priority != 0) {
        return false;
    }

    if (cls_rule_is_catchall(&rule->cr)) {
        return true;
    }

    static struct minimatch pt_match;
    static struct ovsthread_once once = OVSTHREAD_ONCE_INITIALIZER;
    if (ovsthread_once_start(&once)) {
        struct match m = MATCH_CATCHALL_INITIALIZER;
        match_set_default_packet_type(&m);
        minimatch_init(&pt_match, &m);
        ovsthread_once_done(&once);
    }
    return minimatch_equal(&rule->cr.match, &pt_match);
}
