            ...
            const struct vl_mff_map *vl_mff_map)
{
    ...
    while (b.size) {
        ...
        if (error) {
            ...
            /* A prerequisite implies that a data field is being matched, and
             * that implies that the packet type must be known.  As a special
             * case for backward compatibility, OpenFlow does not require an
             * explicit match on the Ethernet packet type, so we infer it. */
